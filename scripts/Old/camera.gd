...
func move_and_zoom(paddles):
    ...
    if paddles.size() > 0:
        ...
        var largest_x = 2 * max(max_x - average.x, average.x - min_x)
        var largest_y = 2 * max(max_y - average.y, average.y - min_y)
        var margin_x = OS.window_size.x * 2 / 3
        var margin_y = OS.window_size.y * 2 / 3
        new_zoom.x = (largest_x + margin_x) / OS.window_size.x
        new_zoom.y = (largest_y + margin_y) / OS.window_size.y
